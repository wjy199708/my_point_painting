...
class KittiDataset(torch.utils.data.Dataset):
    def __init__(self, root, mode, valid):
        ...
        if self.mode == 'training':
            ...
            self.imgs = list(
                sorted(os.listdir(os.path.join(root, mode, "image_2"))))
            self.lidar = list(
                sorted(os.listdir(os.path.join(root, mode, "velodyne"))))
            self.calib = list(
                sorted(os.listdir(os.path.join(root, mode, "calib"))))

    def __getitem__(self, idx):

        img_path = os.path.join(self.root, self.mode,
                                "image_2", self.imgs[idx])
        lidar_path = os.path.join(
            self.root, self.mode, "velodyne", self.lidar[idx])
        calib_path = os.path.join(
            self.root, self.mode, "calib", self.calib[idx])
